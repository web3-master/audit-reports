...
contract AlchemistV3Test is Test {
    ...
    function testMintAndCheckGlobalCollateralRatio() external {
        ...
        uint256 totalLockedUnderlyingValue = alchemist.getTotalLockedUnderlyingValue();
        emit log_named_uint("totalLockedUnderlyingValue", totalLockedUnderlyingValue);

        uint256 debtShares = alchemist.convertDebtTokensToYield(alchemist.totalDebt());
        uint256 requiredDebtShares = FixedPointMath.mulDivUp(debtShares, alchemist.globalMinimumCollateralization(), FIXED_POINT_SCALAR);
        uint256 correctTotalLockedUnderlyingValue = alchemist.convertYieldTokensToUnderlying(requiredDebtShares);
        emit log_named_uint("correctTotalLockedUnderlyingValue", correctTotalLockedUnderlyingValue);

        assert(totalLockedUnderlyingValue != correctTotalLockedUnderlyingValue);
    }
}
